...
| `VIO-007` | Read device-specific config, discover + initialize virtqueues. | Virtio 1.2 §2.1.2 step 7 | implemented | Slice 1 (discovery): QueueSel + QueueNumMax probe for queues 0 (RX) and 1 (TX). Firecracker advertises 0x100 (256) for both. Emits `QUEUES:RX=<hex> TX=<hex>`. Slice 2 (init): for each queue, QueueNum := QueueNumMax, QueueDesc/Driver/Device physical addresses programmed, QueueReady := 1 with read-back verify. Virtqueue memory at `VIRTQ_BASE` (4 MiB), 12 KiB/queue, page-aligned slots for descriptor / available / used rings. Firecracker zero-initializes guest RAM so rings start at `idx=0`, `flags=0` (uninhabited-queue state). Emits `QUEUES:READY` on success, `QUEUES:FAIL queue-not-ready` on read-back mismatch (routes through VIO-009 FAILED). Device-specific config read (MAC) still to come — tracked as `VIO-F-002` deviation. |
| `VIO-008` | Set `DRIVER_OK` (bit 2) in status; device is now live. | Virtio 1.2 §2.1.2 step 8 | implemented | `arch/x86_64/platform/firecracker/boot.S` final step after `QUEUES:READY`. RMW ORs `VIRTIO_STATUS_DRIVER_OK` into Status, then equality-compares read-back against `ACK\|DRIVER\|FEATURES_OK\|DRIVER_OK` (0x0F) — catches stray bits or FAILED set during init. Emits `DRIVER_OK`; mismatch emits `DRIVER_OK:FAIL status=<hex>` and routes through VIO-009. |
