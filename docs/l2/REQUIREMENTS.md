...
| `VIO-007` | Read device-specific config, discover + initialize virtqueues. | Virtio 1.2 §2.1.2 step 7 | spec | Discovery (QueueSel + QueueNumMax) landed in `arch/x86_64/platform/firecracker/boot.S`; emits `QUEUES:RX=<hex> TX=<hex>` marker. Firecracker currently advertises 0x100 (256) for both queues. Memory allocation + QueueNum + QueueDesc/Driver/Device physical-address programming + QueueReady toggle still to come; status remains `spec` until that lands. |
