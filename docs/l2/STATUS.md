...
6. **ARP cache + initiator** — in progress (full state
   ...
   - 6.e ✅ timer + state machine. All transitions wired:
     REACHABLE → STALE (aging), INCOMPLETE → retries →
     FAILED, STALE → PROBE (refresh send), PROBE → retries
     → FAILED. PROBE → REACHABLE is the natural cleanup
     path via the dispatcher RX-reply hook (arp_cache_insert
     overwrites with REACHABLE on a successful reply).
