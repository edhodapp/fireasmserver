...
class TestToolchainForArch:
    ...
    def test_x86_64(self) -> None:
        asm, ld = toolchain_for_arch("x86_64")
        assert asm == "x86_64-linux-gnu-as"
        assert ld == "x86_64-linux-gnu-ld"

    def test_aarch64(self) -> None:
        asm, ld = toolchain_for_arch("aarch64")
        assert asm == "aarch64-linux-gnu-as"
        assert ld == "aarch64-linux-gnu-ld"
...
class TestBuildGuest:
    ...
    def test_no_sources_raises(self, tmp_path: object) -> None:
    ...
    def test_assembles_and_links(
        self, mock_run: MagicMock, tmp_path: object,
    ) -> None:
        src = tmp_path / "src"  # type: ignore[operator]
        src.mkdir()
        (src / "boot.S").write_text(".global _start\n")
        build = tmp_path / "build"  # type: ignore[operator]
        result = build_guest(
