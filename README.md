...
See [DECISIONS.md](DECISIONS.md) for the full architecture decision log (D001–D061). Supersessions carry bidirectional cross-references: the new entry cites what it replaces, and the old entry is strike-through'd with a `DEPRECATED <ISO-8601>Z — see DNNN` marker (established convention since D042).
...
- **Pi 5 local test host** — PiOS Trixie 64-bit custom-built via pi-gen with a KVM-enabled kernel, static `10.0.2.2/24` on USB NIC, pubkey-only SSH, first-user password random-per-build. Firecracker v1.15.1 installed from the upstream prebuilt release (D037 amends D026 during bring-up). `apt-cacher-ng` proxy on the laptop makes the Pi's package world work through a single bridge hop (D035). Two-tier backup strategy (D036) keeps snapshots without SD removal.
